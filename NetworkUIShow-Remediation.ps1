...
try {
    ...
    if ($null -ne $currentValue) {
        if ($currentValue -eq $DesiredValue) {
            ...
            Write-Output "Non-compliant: $ValueName is set to $currentValue. Changing to $DesiredValue..."
            Set-ItemProperty -Path $RegPath -Name $ValueName -Value $DesiredValue -Type DWord
            Write-Output "$ValueName has been remediated to $DesiredValue."
            exit 0
        }
    } else {
        Write-Output "$ValueName not found. Creating and setting to $DesiredValue..."
