...
###Making an array for scalability###
    $existenceArray = 
        @(  @{Attribute="HKLM:\SOFTWARE\NetDocuments"; Value=If($null -ne $existenceCheck1){'OfflineModeNotification'}}
            ...
            @{Attribute="HKLM:\SOFTWARE\NetDocuments"; Value=If($null -ne $existenceCheck11){'UseEmbeddedNetWebBrowser'}}
            @{Attribute="HKCU:\SOFTWARE\NetDocuments"; Value=If($null -ne $existenceCheck12){'UseEmbeddedNetWebBrowser'}})
