# Anyconnect Uninstall 05/07/2024
### Detection
...
foreach ($path in $uninstallKeys) {
    Get-ChildItem $path | ForEach-Object {
        ...
        if ($properties.DisplayName -eq "Cisco AnyConnect Secure Mobility Client") {
            $found = $true
...
if ($found) {
    write-output "AnyConnect Version $version found. Uninstalling."
    exit 1
} else {
    write-output "AnyConnect not found."
    exit 0
}
...
# Define the log path
$logPath = "C:\windows\temp\CiscoAnyconnect-Uninstall.log"
