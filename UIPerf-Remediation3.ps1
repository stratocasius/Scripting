...
CPU Speed     : $($result.CPUGHz) GHz
Memory Used   : $($result.MemoryUsedPct) %
Total RAM     : $($result.TotalMemoryGB) GB
Free Disk     : $($result.DiskFreePct) %
Free Disk GB  : $($result.DiskFreeGB) GB
...
"@ | Out-File -FilePath $txtLog -Encoding utf8 -Force
    ...
    Write-Output "$($result.Score) - UIPerfScore | Grade - $($result.Grade) | CPU % - $($result.CPUPercent) | RAM % - $($result.MemoryUsedPct) | Free Disk % - $($result.DiskFreePct) | Issues: ""$issuesText"" | Trend - $($priorTrend.TrendStatus) | TrendDelta - $([math]::Abs($priorTrend.Delta)) | TrendCount - $($priorTrend.HistoryCount) | $ramText * $cpuGHzText * $diskFreeText |"
    exit 0
}
catch {
