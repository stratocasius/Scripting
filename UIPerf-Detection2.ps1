...
try {
    ...
    Write-Output "UIPerfScore=$($result.Score);Grade=$($result.Grade);CPU=$($result.CPUPercent);MemoryUsed=$($result.MemoryUsedPct);DiskFree=$($result.DiskFreePct);DiskQueue=$($result.DiskQueue);Issues=$($result.Issues);Trend=$($trend.TrendStatus);TrendDelta=$($trend.TrendDelta);TrendHistoryCount=$TrendHistoryCount;LogPath=$logRoot;Time=$($result.TimeStamp)"
    ...
    Write-Output "UIPerfScore=0;Grade=F;Issues=AssessmentFailed;Trend=Unknown;TrendHistoryCount=$TrendHistoryCount;LogPath=$logRoot;Error=$($_.Exception.Message)"
    exit 1
}