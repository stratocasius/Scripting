...
if ($null -ne $RegistryValue) {
    ...
    if ($CurrentValue -eq $DesiredValue) {
        Write-Host "ndMail Addin DWORD value is currently set to $DesiredValue."
        exit 0  # No remediation is required.
    } else {
        Write-Host "ndMail addin is Disabled and set to $CurrentValue. Remediation needed"
        exit 1  # This exit code signals that a remediation is needed.
    }
} else {
    Write-Host "The registry DWORD value does not exist. ndMail not installed"
    exit 0 
}