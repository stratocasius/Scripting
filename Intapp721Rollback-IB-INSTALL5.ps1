...
### Begin Intapp Time 7.2.1.300 Installers.
################################################################################################################################
# Start transcript
...
Start-Process -FilePath "msiexec.exe" -ArgumentList "/i `"IntappTimeSetup721.msi`" /qn /norestart /l*v `"C:\ProgramData\Microsoft\IntuneManagementExtension\Logs\IntappTime721Rollback-INSTALL.log`"" -Wait -NoNewWindow
...
& cmd /c "setup64.exe" /verysilent /norestart /DEDestinationDirectory="C:\Program Files\Intapp\Desktop Extension" /LaunchDE=true /TBAPIServiceURL="https://time.jacksonlewis.com:8080/APIService" /CreateDEStartMenuShortcut=false /CreateTBStartMenuShortcut=false /Log=C:\ProgramData\Microsoft\IntuneManagementExtension\Logs\IntappDE721Rollback-INSTALL.log
