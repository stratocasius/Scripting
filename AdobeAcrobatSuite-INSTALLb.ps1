### Adobe Acrobat 25.001 Suite for Autopilot - 12/08/2025
### Includes removal/update of ndOffice Suite. 
### REMINDER - Edit ndOfficeInstall421.exe below back to 401.exe before deploying!!!!

$logFilepath = "$($env:ProgramData)\Microsoft\IntuneManagementExtension\Logs\AdobeAcrobatSuite-Transcript.log"
### Start logging
...
#        $displayName = Get-ItemProperty $_.PSPath | Select-Object -ExpandProperty DisplayName -ErrorAction SilentlyContinue
 ...
 #           # Log details
 #           Write-Output "Found $displayName with version $displayVersion, Uninstalling"
 #           Start-Process -Wait "$UninstallString" -ArgumentList "/quiet /norestart /log C:\Programdata\Microsoft\IntuneManagementExtension\Logs\ndOfficeInstaller-UNINSTALL.log"
 #           }
 #              Write-Output "Successfully uninstalled ndOffice Setup at $(Get-Date)."
 #              
 #       }
 #   }
...
# Adding gccustomhook.exe to AV process exclusion
Write-Output "Adding gccustomhook.exe to antivirus exclusion list..."
Add-MpPreference -ControlledFolderAccessAllowedApplications "gccustomhook.exe"
Write-Output "gccustomhook.exe added to antivirus exclusion list."

$setupPath = "C:\jltools\AdobeAcrobatSuite\setup.exe"
if (Test-Path $setupPath) {
    Set-Location -Path "C:\jltools\AdobeAcrobatSuite"
    Start-Process "C:\jltools\AdobeAcrobatSuite\setup.exe" -ArgumentList "--silent" -Wait
...
### Removing gccustomhook.exe to AV process exclusion
Write-Output "Removing gccustomhook.exe from antivirus exclusion list..."
Remove-MpPreference -ControlledFolderAccessAllowedApplications "gccustomhook.exe"
Write-Output "gccustomhook.exe removed from antivirus exclusion list."
#######################



#region ndOffice 4.2.1 app installations
$setupPath = "C:\jltools\AdobeAcrobatSuite\ndOfficeSetup421.exe"
if (Test-Path $setupPath) {
    Start-Process "ndOfficeSetup.exe" -ArgumentList "/install", "/quiet", "/norestart" -Wait
    Write-Host "Installation Start-Process -FilePath $setupPathof ndOfficeSetup.exe completed at $(get-Date)."
} else {
