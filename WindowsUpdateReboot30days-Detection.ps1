...
try {
    ...
    Write-Warning "Log cleanup failed in $LogDir : $_"
}

# Start transcript logging
try {
    Start-Transcript -Path $LogPath -Force -ErrorAction Stop | Out-Null
    Write-Output "=== WUfB Reboot Detection Script Started: $(Get-Date -Format u) ==="
    Write-Output "Log file: $LogPath"
    if ($cutoff) {
        Write-Output "Retention: $LogRetentionDays day(s); removed $($oldLogs.Count) old log(s) older than $($cutoff.ToString('u'))."
    }
}
catch {
...
function Get-UpdateInfoFromWUEvent {
    ...
    # Resolve Title (PowerShell 5.1 compatible — no ??)
    $title = $null
    ...
    if (-not $title) {
        $title = ($Event.Message -split "`r?`n")[0]
    }
...
# Nice table for console viewing
...
$summary = (
    $joined | ForEach-Object {
        ...
        "[Reboot: $($_.RebootTime.ToString('u')) | KB(s): $kb | Confidence: $($_.Confidence)]"
    }
) -join ' ; '
...
try {
    Write-Output "=== WUfB Reboot Detection Script Completed: $(Get-Date -Format u) ==="
