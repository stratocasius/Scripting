### Uninstall Script for ResearchMonitor Web Extension 3.6.1.629 - 03/10/2025
### Establish transcript
$logFilepath = "$($env:ProgramData)\Microsoft\IntuneManagementExtension\Logs\RMWebExt3.6.1.629-Removal.log"
### Start logging
Start-Transcript "$($env:ProgramData)\Microsoft\IntuneManagementExtension\Logs\RMWebExt3.6.1.629-Removal.log"
...
foreach ($path in $uninstallPaths) {
    ...
    Get-ChildItem -Path $path -ErrorAction SilentlyContinue | ForEach-Object {
        ...
        if ($displayName -and $uninstallString) {
            if ($displayName -match "ResearchMonitor Web Extension Support Files - 3.6.1") {
