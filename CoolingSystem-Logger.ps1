### Cooling System Monitoring Script - 02/23/2026

$ErrorActionPreference = 'Continue'

$OutCsv          = 'C:\programdata\microsoft\IntuneManagementExtension\Logs\CoolingSystem.csv'
...
while ((Get-Date) -lt $end) {
    ...
    # --- ACPI Thermal Zone Information (often drives "Thermal Zone Information" counters) ---
    # Namespace root\wmi, class MSAcpi_ThermalZoneTemperature (CurrentTemperature is tenths of Kelvin)
    try {
        ...
        Write-Row "MSAcpi_ThermalZoneTemperature" "Error" "$(.Exception.Message)"
    }

    # --- Fan devices (often empty on laptops depending on vendor SMBIOS exposure) ---
    try {
        ...
        foreach ($f in $fans) {
            ...
            # DesiredSpeed / CurrentSpeed often not populated
        ...
        Write-Row "Win32_Fan" "Error" "$(.Exception.Message)"
    }

    # --- Temperature probes (often empty depending on SMBIOS support) ---
    try {
        ...
        Write-Row "Win32_TemperatureProbe" "Error" "$(.Exception.Message)"
    }
