# VisualEffectsNerf-Detection3.ps1 - 02/25/2026
# Detection: Windows 11 Visual Effects Detection (PR) - FAST TRACK (exit on first mismatch)
...
foreach ($c in $checks) {
    ...
    if ($null -eq $actual) {
        Write-Output ("NonCompliant|{0}|{1}=<missing>" -f $UserName, $c.Name)
        exit 1
    }
    ...
    if ($a -ne $e) {
        Write-Output ("NonCompliant|{0}|{1}={2} (exp {3})" -f $UserName, $c.Name, $a, $e)
        exit 1
    }
}

Write-Output ("Compliant | {0} | AllSettings OK" -f $UserName)
