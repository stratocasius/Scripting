...
foreach ($member in $groupMembers) {
    ...
    # Filtering for Windows devices using an additional filter condition (### CHANGED FROM azureADDeviceId to ObjectID)
    $intuneDevices = Get-IntuneManagedDevice -Filter "userPrincipalName eq '$($member.UserPrincipalName)' and operatingSystem eq 'Windows'" | Select-Object id, userPrincipalName, deviceName, ObjectID
    ...
    foreach ($device in $intuneDevices) {
        ...
        $results += [PSCustomObject]@{
            ...
            'AzureADDeviceId' = $device.ObjectID
        }
    }
}
