### Uninstall Script for Poll Everywhere - 01/21/2025
### Establish transcript
$logFilepath = "$($env:ProgramData)\Microsoft\IntuneManagementExtension\Logs\Poll Everywhere-Removal.log"
### Start logging
Start-Transcript "$($env:ProgramData)\Microsoft\IntuneManagementExtension\Logs\PollEverywhere-Removal.log" -Append
...
foreach ($path in $uninstallPaths) {
    ...
    Get-ChildItem -Path $path -ErrorAction SilentlyContinue | ForEach-Object {
        ...
        # Check if DisplayName contains "Poll Everywhere" and Publisher equals "Poll Everywhere potenially"
        if ($displayName -and $uninstallString) {
            if ($displayName -match "Poll Everywhere") {
