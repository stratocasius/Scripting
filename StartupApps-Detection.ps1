...
function Get-EstimatedStartupImpact {
    ...
    # Heuristic only, not the exact Task Manager startup-impact metric
    $highPatterns = @(
        'teams.exe',
        'msteams.exe',
        'outlook.exe',
    ...
    # Reasonable defaults by source
    if ($Source -eq "Startup") {
        return "Medium"
    }

    if ($Source -eq "HKLM" -or $Source -eq "HKCU") {
        return "Low"
    }
...
try {
    ...
    if (-not $apps -or $apps.Count -eq 0) {
        ...
        Write-Output "$user | Startup apps - None found"
        exit 0
    }
    ...
    $output = $apps |
        ...
        ForEach-Object { "$($_.Exe) ($($_.Source), $($_.Impact))" }

    $summary = "$user | Startup apps - $($output -join ', ')"
    ...
    Write-Output "$user | Startup apps - Detection failed"
    exit 1
}