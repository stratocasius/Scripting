...
- Microsoft-Windows-AppModel-Runtime/Admin : generic activation traces
- Optional: C:\Windows\Prefetch\OLK.EXE-*.pf timestamp (fallback; may not exist for packaged apps)
...
# Target identifiers
$TargetPackageHint = 'Microsoft.OutlookForWindows'  # package family name for the new Outlook
$TargetExeHint     = 'olk.exe'                      # the exe in %LOCALAPPDATA%\Microsoft\WindowsApps\olk.exe
...
Write-Log "Lookback window: last $LookbackDays day(s) since $($StartTime.ToString('yyyy-MM-dd HH:mm:ss'))"
Write-Log "Searching for launches of new Outlook (PackageHint='$TargetPackageHint', ExeHint='$TargetExeHint')."
...
# Not guaranteed for packaged apps, but cheap to check.
try {
...
if ($matches -and $matches.Count -gt 0) {
    # Summarize latest hit
    $latestEvt = ($matches | Where-Object { $_ -is [System.Diagnostics.Eventing.Reader.EventRecord] } |
                  Sort-Object TimeCreated -Descending | Select-Object -First 1)

    if ($latestEvt) {
        $userSid = $null
        try { $userSid = $latestEvt.UserId.Value } catch {}
        Write-Log ("Latest evidence: Log={0}, Id={1}, Time={2}, UserSid={3}" -f $latestEvt.LogName, $latestEvt.Id, $latestEvt.TimeCreated, ($userSid ?? 'N/A'))
        # Print first line of message to keep console clean
        $firstLine = (($latestEvt.Message -split '\r?\n')[0])
        Write-Log ("Msg: {0}" -f $firstLine)
        Write-Output ("Outlook (new) launch detected within {0} days; latest: {1:yyyy-MM-dd HH:mm:ss} (Log={2}, Id={3})" -f $LookbackDays, $latestEvt.TimeCreated, $latestEvt.LogName, $latestEvt.Id)
    } else {
