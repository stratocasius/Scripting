### Uninstall Script for Litera Metadact 5.18 - 04/26/2025
### Establish transcript
# Track script start time (before anything else happens)
$global:ScriptStartTime = Get-Date
$logFilepath = "$($env:ProgramData)\Microsoft\IntuneManagementExtension\Logs\Metadact5.18-Removal.log"
### Start logging
...
foreach ($path in $uninstallPaths) {
    ...
    Get-ChildItem -Path $path -ErrorAction SilentlyContinue | ForEach-Object {
        ...
        # Check if DisplayName contains "Metadact5.18" and Publisher equals "Metadact5.18 potenially"
        if ($displayName -and $uninstallString) {
            if ($displayName -match "Litera Metadact") {
